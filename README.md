...
Además del encadenamiento criptográfico, el sistema cuenta con **Integridad Cruzada (Context-Aware Validation)**. La bitácora no es "ciega" a lo que protege: si un registro afirma que se creó un reporte (ej. "Nueva denuncia con ID 5"), la función de verificación irá a la tabla de reportes a comprobar que el reporte número 5 realmente exista. Si un administrador malicioso intenta borrar físicamente una denuncia de la base de datos, la cadena de hashes cuadrará, pero la validación cruzada detectará que el archivo desapareció y disparará la alarma visual marcando el log con `[✗ REPORTE ELIMINADO/FALTANTE]`.
...
5. **Zero-Disk Trace y Trazabilidad:** Para evitar fugas de información, los archivos PDF e imágenes se renderizan directamente en la memoria RAM del navegador, evitando que el auditor deba guardarlos en su disco duro. Adicionalmente, si el auditor decide forzar la descarga de una evidencia, el sistema notifica de forma silenciosa al servidor, registrando un evento inmutable (`EVIDENCE_DOWNLOADED`) en la bitácora de auditoría.

### 5.3 Flujo del administrador

1. Inicia sesión (`admin` / `SafeDropAdmin2026!`).
2. Pulsa "Verificar Integridad": el sistema recalcula toda la cadena de hashes SHA-256, aplica la validación cruzada para asegurar que ningún reporte haya sido borrado, y muestra "Integridad Asegurada" si todo coincide.
...
4. Pulsa "Simular Borrado": se elimina físicamente un reporte de la base de datos, saltándose la API, para demostrar el funcionamiento de la Integridad Cruzada.
5. Al volver a pulsar "Verificar Integridad", el sistema detecta cualquiera de estas discrepancias y muestra una alerta visual señalando exactamente qué registro fue comprometido o qué reporte fue borrado de forma irregular.
